...
// CommonLoadingScreenSettings.h (From Lyra, 장주만 수정)
...
/* JM : 맵 이름과 위젯 경로를 매핑하고 이동할 레벨마다 다른 로딩 화면을 띄우기 위함 */
USTRUCT(BlueprintType)
struct FLoadingScreenMapping
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, Category = Config)
	FString MapKeyword;		// 맵 이름 키워드(Lobby, Stage1 등), 이동하려는 맵 이름에 이 문자열이 포함되어 있으면 해당 위젯을 사용함

	UPROPERTY(EditAnywhere, Category = Config, meta = (MetaClass = "/Script/UMG.UserWidget"))
	FSoftClassPath LoadingScreenWidget;		// 해당 맵 로딩 시 띄울 위젯
};
...
public:
	FSoftClassPath GetLoadingScreenWidgetPathForMap(const FString& MapName) const;		// JM 추가

public:
	UPROPERTY(Config, EditAnywhere, Category=Display, meta=(MetaClass="/Script/UMG.UserWidget"))
	FSoftClassPath DefaultLoadingScreenWidget;		// JM 추가

	UPROPERTY(Config, EditAnywhere, Category=Display)
	TArray<FLoadingScreenMapping> LoadingScreenMappings;	// JM 추가
	
	// The widget to load for the loading screen.
	// UPROPERTY(config, EditAnywhere, Category=Display, meta=(MetaClass="/Script/UMG.UserWidget"))
	// FSoftClassPath LoadingScreenWidget;		// JM 원본 삭제
